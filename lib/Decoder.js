...
Decoder.prototype.read = function read(offset, numberOfBytes) {
    ...
    if (numberOfBytes === 1) {
        return Buffer.from([this.fileStream[offset]]);
    }
...
Decoder.prototype.decodePointer = function decodePointer(ctrlByte, offset) {
    ...
    packed = (pointerSize === 4) ? buffer : Buffer.concat([Buffer.from([ctrlByte & 0x7]), buffer], buffer.length + 1);
...
Decoder.prototype.decodeBigUint = function decodeBigUint(bytes, size) {
    ...
    for (i; i < numberOfLongs; i++) {
        integer = BigInt(integer) * BigInt(4294967296) + BigInt(buffer.readUInt32BE(i << 2, true));
    }
